...
K=2; # Dimensi�n de X (n�mero de regresores)
...
e=zeros(N,1)+sigma*randn(N,1);

#%
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#PREGUNTA1: Variables explicativas con menor y mayor variaci�n
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

sd1=0.001; # desviaci�n est�ndar para el caso con baja variaci�n
sd2=1; # desviaci�n est�ndar para el caso con alta variaci�n

#simular los datos
X1=[ones(N,1) mediaX*ones(N,K)+ (sd2*randn(N,K))];
X2=[ones(N,1) mediaX*ones(N,K)+ (sd1*randn(N,K))];
...
E_X2=mean(X2);
Sd_X2=sqrt(var(X2));
%Comprobar que no hay correlaci�n en los datos generados(deber�a
%aproximarse a cero)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%definimos la correlaci�n a generar
co=0.95;
%simular datos correlacionados(usamos propiedades de la normal para
%mantener la desviaci�n est�ndar y la media requeridas)
X3=zeros(N,3);
X3(:,1)=ones(N,1);
...
%Comprobar la media y desviaci�n est�ndar de los datos generados
E_X3=mean(X3);
Sd_X3=sqrt(var(X3));
...
%normal para mantener la media y desviaci�n est�ndar)
X4=zeros(N,3);
X4(:,1)=ones(N,1);
...
%Calcular sesgo con el modelo sin correlaci�n con el error
